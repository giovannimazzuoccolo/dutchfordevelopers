...
const actions: ActionTree<any, any> = {
    ...
    async getGamesWithScore() { 
        ...
        const { data, error } = await supabase.from("games").select("*");
        if (!error) {
            const { data:scores, error:scoresErr } = await supabase.from("scores").select("game,score").eq("user_id", userInfo.id);
            if(!scoresErr) {
                this.commit('games/REQUEST_SUCCESS');
                const gamesWithScore = data.map(d => ({...d, score: scores?.find(s => s.game === d.route)?.score }));
